...
public:
     ...
     * base_pooling_matrix_ The pooling matrix for the test bundles i.e. the left regular graph for test bundles. This also just an alias for pools_.
     * permutation_map_ Contains the permuations for the item index.
     * signature_matrix_ vector<vector<bool>> signature_matrix[item_id] has full signature of item.
     * 
     * @param debug Debug level.
    ...
    inline set<uint> peelingAlgorithm(vector<vector<bool>> residuals, set<uint> identified_defectives, int debug = 0) {
        ...
        while(!identified_defectives.empty() && !unresolved_pools.empty()){
            ...
            for(const uint &pool_id : base_pooling_matrix.items_to_pools[item_id]){
                ...
                int code = decodeSignature(residuals[pool_id], permutation_map_);
                if(code >= 0 ){
                ...
                // Doubleton/ Multiton Peel now and see what is remaining. 
                    ...
                    vector<bool> result = peelSignature(residuals[pool_id], signature_matrix_, item_id);
                    code = decodeSignature(result, permutation_map_);
            ...
            for(uint i = 0; i < unresolved_pools.size(); i++ ){
                ...
                while(!identified_defectives.empty()){
                    ...
                    for(uint pid : base_pooling_matrix.items_to_pools[item_id]){
                        ...
                        vector<bool> result = peelSignature(residuals[pid], signature_matrix_, item_id);
                        code = decodeSignature(result, permutation_map_);
                        if(code >= 0 ){
                            if(identified_defectives.find(code) == identified_defectives.end() && defective_items.find(code) == defective_items.end()){
                                identified_defectives.insert(code);
...
protected:
    ...
    inline vector<bool> getSignature( const vector<uint>& permutation_map) {
        ...
        signature.reserve(num_permutations * (2 * L_));  
